...
function activate_conda_env() {
  ...
  if [ -z "$env_name" ] || [ -z "$python_version" ]; then
      echo "Error: Environment name and Python version must be provided."
      return 1
  fi
  ...
  if [ -z "$ENV_EXISTS" ]; then
      # Create the conda environment with the specified Python version if it doesn't exist
...
# Function to find the conda.sh script and source it
initialize_conda() {
    # Try to find conda.sh in typical install locations
    local conda_setup_script_paths=(
    ...
    for conda_setup_script in "${conda_setup_script_paths[@]}"; do
        if [[ -f "$conda_setup_script" ]]; then
            echo "Sourcing Conda from: $conda_setup_script"
            source "$conda_setup_script"
            return 0
        fi
    done

    echo "Error: Unable to find conda.sh. Please check your Conda installation."
    return 1
}

# Check if the conda command is available, if not, try to initialize it
